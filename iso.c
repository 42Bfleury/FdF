...
/*   Updated: 2016/11/03 13:17:02 by bfleury          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void		free_iso_line(t_point *p1, t_point **p, t_color **c)
{
	free(p1);
	free(p[0]);
	free(p[1]);
	...
	while (++(p->y) < e->map.height)
	{
		...
		while (++(p->x) < e->map.width)
		{
			if (p->x < e->map.width - 1)
				iso_line(e, p, get_point(p->x + 1, p->y));
			if (p->y < e->map.height - 1)
				iso_line(e, p, get_point(p->x, p->y + 1));
		}
	}
	free(p);
